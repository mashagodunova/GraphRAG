...
class Memoization:
    ...
    def get_best_move(self, node):
        return self.best_moves.get(node, None)

    def set_best_move(self, node, move):
        self.best_moves[node] = move
...
@beartype
def hierarchical_leiden_with_memoization(
    graph: Union[
        List[Tuple[Any, Any, Union[int, float]]],
        nx.Graph,
        np.ndarray,
    ...
    """
    Hierarchical Leiden with memoization and handling of isolated nodes.
    """
    _validate_common_arguments(
        extra_forced_iterations,
        resolution,
    ...
    # Memoization object for best moves
    ...
    for entry in hierarchical_clusters_native:
        ...
        # Update memoization for the node
        node = partition.node
        current_cluster = partition.cluster
        best_move = memoization.get_best_move(node)
        if best_move is None or current_cluster != best_move:
            # Recompute the best move if necessary
            best_move = compute_best_move_for_node_in_hierarchy(
                node, native_friendly_communities, edges, resolution
            )
            memoization.set_best_move(node, best_move)

    # Handle isolated nodes
    all_input_nodes = set(range(node_count))
    isolated_nodes = all_input_nodes - all_nodes
    for isolated_node in isolated_nodes:
        # Add isolated nodes as their own communities
        result_partitions.append(
            HierarchicalCluster(
                node=identifier.original(isolated_node),
                cluster=-1,  # Mark as isolated
                parent_cluster=None,
                level=0,
    ...
    if len(all_nodes) < node_count:
        warnings.warn(
            "Leiden partitions do not contain all nodes from the input graph. "
            "Isolated nodes have been added as separate entries."
        )

    return result_partitions


def compute_best_move_for_node_in_hierarchy(node, communities, edges, resolution):
    """
    Computes the best move for a node in the hierarchical Leiden algorithm.
    """
    # Placeholder logic to compute the best cluster for the node
    # This would be replaced by the actual logic for hierarchical clustering.
    return communities.get(node, -1) 