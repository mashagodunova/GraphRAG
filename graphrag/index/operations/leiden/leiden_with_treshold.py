...
@beartype
def hierarchical_leiden_threshold(
    graph: Union[
        List[Tuple[Any, Any, Union[int, float]]],
        nx.Graph,
        np.ndarray,
    ...
    weight_default: Union[int, float] = 1.0,
    check_directed: bool = True,
    weight_threshold: float = 0.5  # New parameter for minimum edge weight to re-add nodes
) -> HierarchicalClusters:
    """
    Modified hierarchical Leiden algorithm that prioritizes adding nodes back into consideration if the edge
    weight between them is above a specified threshold, ensuring stronger connections are re-examined.
    """
    _validate_common_arguments(
        extra_forced_iterations,
        resolution,
    ...
    for entry in hierarchical_clusters_native:
        ...
        if any(
            edge[2] > weight_threshold and (edge[0] == str(partition.node) or edge[1] == str(partition.node))
            for edge in edges
        ):
